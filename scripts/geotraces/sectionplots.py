...
text = {'ws': 'Settling speed, < 51 µm (m d$^{-1}$)',
        'wl': 'Settling speed, > 51 µm (m d$^{-1}$)',
        'B2p': 'Aggregation (m$^{3}$ mmol$^{-1}$ d$^{-1}$)',
        'Bm2': 'Disaggregation (d$^{-1}$)',
        'Bm1s': 'Remineralization, < 51 µm (d$^{-1}$)',
        'Bm1l': 'Remineralization, > 51 µm (d$^{-1}$)',
        'sinkdiv_S': 'Sinking flux divergence, < 51 µm (mmol m$^{-2}$ d$^{-1}$)',
        'sinkdiv_L': 'Sinking flux divergence, > 51 µm (mmol m$^{-2}$ d$^{-1}$)',
        'remin_S': 'Remineralization, < 51 µm (mmol m$^{-2}$ d$^{-1}$)',
        'remin_L':'Remineralization, > 51 µm (mmol m$^{-2}$ d$^{-1}$)',
        'aggregation': 'Aggregation (mmol m$^{-2}$ d$^{-1}$)',
        'disaggregation': 'Disaggregation (mmol m$^{-2}$ d$^{-1}$)',
        'production': 'Production (mmol m$^{-2}$ d$^{-1}$)',
        'dvm': 'DVM (mmol m$^{-2}$ d$^{-1}$)'}
...
def plot_section(df, suffix):

    if suffix in ('sinkdiv_S', 'sinkdiv_L', 'dvm'):
        scheme = plt.cm.PRGn
        # bounds = (min(df['estimate']), max(df['estimate']))
        normfac = TwoSlopeNorm(0, min(df['estimate']), max(df['estimate']))
    ...
    fig.suptitle(text[suffix], fontsize=16)
    plt.savefig(f'../../results/geotraces/section_{suffix}')
...
for f in pickled_files:
    ...
    with open(file_path, 'rb') as file:
        ...
        _, _, _, _, _, _, _, grid, zg, mld, *_ = unpickled
...
for p in dv_params:
    ...
    for f in pickled_files:
        ...
        with open(file_path, 'rb') as file:
            unpickled = pickle.load(file)
            _, params, _, _, _, _, _, grid, _, _, layers, *_ = unpickled
...
for i in int_fluxes:
    ...
    for f in pickled_files:
        ...
        with open(file_path, 'rb') as file:
            unpickled = pickle.load(file)
            _, _, _, _, int_fluxes, _, _, grid, zg, _, layers, *_ = unpickled
