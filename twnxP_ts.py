...
Twin experiments with many groups of target values to test sensitivity of results on choice of target values ('_ts' = target sensitivity).
...
ptargs = {}
for row in tgt_dl:
    g, p, v = int(row['group']), row['param'], float(row['val'])
    if g not in ptargs.keys(): ptargs[g] = {p:{} for p in params}
    if '_' in p:
        p, l = p.split('_')
        ptargs[g][p][l] = v
    else: ptargs[g][p] = v
...
for g in ptargs.keys():
    ...
    for i in np.arange(0,P):
        ...
        wsi, wli, Bm2i, Bm1si, Bm1li, Ghi = ptargs[g]['ws'][l], ptargs[g]['wl'][l], \
            ptargs[g]['Bm2'][l], ptargs[g]['Bm1s'][l], ptargs[g]['Bm1l'][l], Ghz[int(d)]
        #POC, SSF
        if t == 'Ps':
            A[i,iPli] = -Bm2i
            b[i] = Ghi
