...
def load_targets(inversion, gamma, rel_err):
    ...
    path = '../../results/exports'

    with open(f'{path}/{inversion}_{rel_err}_{gamma}.pkl', 'rb') as pickled:
...
def generate_nonlinear_solution(targets):
    ...
    for layer in LAYERS:
        ...
        if l == 0:
            ...
            b[l] = -Lp*Po*(np.exp(-(zim1 - MLD)/Lp) - np.exp(-(zi - MLD)/Lp))
...
def generate_pseudodata(targets, priors_from, gamma, rel_err):

    targets = load_targets(priors_from, gamma, rel_err)
